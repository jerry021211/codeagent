...
export function WorkspacePicker(props: Props) {
  ...
  useEffect(() => {
    ...
    else { setPath(""); setEditing(false); }
  }, [props.open, props.initialPath]);

  useEffect(() => {
    if (props.open && !editing && props.listing?.current) setPath(props.listing.current);
  }, [props.open, props.listing?.current]);

  useEffect(() => {
    if (!props.open || !editing) return;
    const timer = window.setTimeout(() => props.onSearch(path.trim()), 250);
    return () => window.clearTimeout(timer);
  }, [props.open, props.onSearch, path, editing]);
    ...
    <div className="fixed inset-0 z-[80] grid place-items-center bg-black/55 p-4 backdrop-blur-sm" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && props.onClose()}>
      <section role="dialog" aria-modal="true" aria-labelledby="workspace-picker-title" className="flex max-h-[min(760px,92vh)] w-full max-w-2xl flex-col overflow-hidden rounded-2xl border border-line bg-surface shadow-2xl">
        ...
        <div className="border-b border-line px-5 py-3">
          <form className="flex gap-2" onSubmit={(event) => { event.preventDefault(); setEditing(true); props.onSearch(path.trim()); }}>
            <input value={path} onChange={(event) => { setPath(event.target.value); setEditing(true); }} aria-label="工作区路径或目录名" spellCheck={false} className="h-10 min-w-0 flex-1 rounded-xl border border-line bg-surface-muted px-3 font-mono text-[11px] text-ink outline-none transition focus:border-accent focus:ring-2 focus:ring-accent/10" placeholder="输入路径或部分目录名，模糊匹配" />
            <button type="submit" disabled={!path.trim() || props.loading} className="rounded-xl border border-line-strong px-4 text-xs font-medium text-ink transition hover:bg-surface-muted disabled:opacity-50">搜索</button>
          </form>
          <p className="mt-2 text-[10px] text-ink-muted">支持部分名称和跳字匹配，忽略大小写。点击匹配目录后打开。</p>
        ...
        <div className="scrollbar-thin min-h-0 flex-1 overflow-y-auto p-3">
          ...
          {!props.loading && props.listing && props.listing.entries.length === 0 && <div className="grid min-h-40 place-items-center text-xs text-ink-faint">{editing && path.trim() ? "没有匹配的目录，请尝试更短的名称" : "这个目录下没有子目录"}</div>}
        </div>
